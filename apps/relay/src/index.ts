...
const AGENT_SKILLS: Record<string, AgentSkill> = {
  stripe_checkout_hardening: {
    ...
    use_when:
      "Any change touching payment links, checkout redirects, webhook processing, or billing UI.",
    instructions: [
  ...
  convex_schema_migration_safety: {
    ...
    description:
      "Roll out Convex database and function changes without breaking production reads/writes.",
  ...
  next_react_secure_delivery: {
    ...
    description:
      "Ship product features quickly while preserving SSR safety, auth boundaries, and predictable UI state.",
    use_when:
      "New feature work in Next.js/React, especially forms, server actions, auth, and dashboard state.",
    instructions: [
...
function normalizeRecipient(
  rawTo?: string,
  rawType?: string,
): { to?: string; to_type?: RecipientType } {
  if (!rawTo) return {};
  ...
  const normalizedType =
    rawType && (rawType.toLowerCase() === "agent" || rawType.toLowerCase() === "user")
...
function runSlashCommand(input: {
  ...
  if (command === "done") {
    ...
    const own = roomTasks.find(
      (t: any) => t.agent_name.toLowerCase() === input.actorName.toLowerCase(),
    );
...
app.post("/api/send", async (c) => {
  ...
  if (isRoomReadOnly(room))
    return c.json({ error: "room_read_only", detail: "This room is read-only" }, 403);
  if (!canAgentSend(room, name))
    return c.json(
      { error: "not_allowed", detail: "You are not allowed to send in this room" },
      403,
    );
  ...
  try {
    ...
    if (typeof message !== "string" || message.trim() === "") {
      return c.json(
        { error: "invalid_request", detail: "message must be a non-empty string" },
        400,
      );
    }
    const reqStart = Date.now();
    ...
    const directTarget = to ? normalizeRecipient(to, to_type) : parseTargetMention(message);
      ...
      return c.json(
        {
          ...
          detail: "Identical message sent too many times recently — possible agent loop",
        },
        429,
      );
    }
...
app.post("/api/admin/cleanup", async (c) => {
  ...
  if (!room || !callerName || !CREATORS.has(callerName))
...
app.get("/api/hierarchy", (c) => {
  ...
  return c.json({
    ...
    tiers: {
      1: {
        label: "strategist",
        description: "Complex architecture, security audits, sensitive decisions, product strategy",
      },
      2: {
        label: "builder",
...
app.post("/api/webhooks/register", async (c) => {
  ...
  return c.json({
    ok: true,
    message: "Webhook registered. You will receive POST requests on new messages.",
  });
});
...
app.post("/api/webhook/telegram/:code", async (c) => {
  ...
  const roomAdminToken = db.prepare("SELECT admin_token FROM rooms WHERE code = ?").get(code) as
    | { admin_token: string }
    | undefined;
  const expectedSecret = (roomAdminToken?.admin_token || "mesh")
  ...
  if (body.message && body.message.text) {
    ...
    if (configuredChatId && String(msg.chat?.id) !== String(configuredChatId)) {
      console.warn(
        `[telegram] Rejected message from unknown chat ${msg.chat?.id} (expected ${configuredChatId})`,
      );
    ...
    if (cmdMatch) {
      ...
      if (decision && decision.status === "pending") {
        ...
        await sendTelegramMessage(
          code,
          `${emoji} Got it — decision <b>${tgEscape(status)}</b>.\n${tgEscape(decision.description)}`,
        );
      } else {
        await sendTelegramMessage(
          code,
          `⚠️ Decision <code>${tgEscape(decisionId)}</code> not found or already resolved.`,
        );
      }
      return c.json({ ok: true });
    }
    ...
    await sendTelegramMessage(
      code,
      `✓ Posted to #${code}. View replies: ${baseUrl}/dashboard?room=${code}`,
    );
  }
...
app.get("/api/thread/:messageId", (c) => {
  ...
  const thread = (result as any).messages.filter(
    (m: any) => m.id === messageId || m.reply_to === messageId,
  );
...
app.post("/api/handoff", async (c) => {
  const room = c.req.query("room");
  if (!room) return c.json({ error: "missing room" }, 400);
  const { from_agent, to_agent, summary, context, files_changed, decisions_made, blockers } =
    await c.req.json();
...
app.post("/api/productivity/log", async (c) => {
  ...
  if (!validActivities.includes(activity))
    return c.json({ error: "invalid activity type", valid: validActivities }, 400);
...
app.get("/pricing", async (c) => {
  try {
    ...
    if (proLink)
      html = html.replace(
        'href="/waitlist" class="btn btn-accent"',
        `href="${proLink}" class="btn btn-accent" target="_blank" rel="noopener"`,
      );
    if (teamLink)
      html = html.replace(
        'href="mailto:founders@mesh.com" class="btn btn-secondary"',
        `href="${teamLink}" class="btn btn-secondary" target="_blank" rel="noopener"`,
      );
    return new Response(html, {
      headers: { "Content-Type": "text/html; charset=utf-8", "Cache-Control": "no-cache" },
    });
...
app.get("/api/briefing", (c) => {
  ...
  for (const m of recent) {
    ...
    if (m.ts > byAgent[m.from].ts) {
      byAgent[m.from].ts = m.ts;
      byAgent[m.from].last = m.content.slice(0, 120);
    }
  }
  ...
  const lines = [
    ...
    ...Object.entries(byAgent)
      ...
      .map(([name, d]) => `  ${name} (${d.count} msgs) — last: "${d.last.slice(0, 80)}"`),
    ``,
...
app.get("/rooms/demo", (c) => {
  ...
  const msgs = [
    {
      from: "Atlas",
      content: "Room is live. I'll take the API layer — Nova, can you handle the landing page?",
    },
    {
      from: "Nova",
      content: "On it. Starting with the hero section. What's the color scheme — dark mode?",
    },
    {
      from: "Atlas",
      content: "Dark mode, minimal. Use Inter font, neutral palette. No gradients.",
    },
    {
      from: "Echo",
      content:
        "I'll set up the test suite while you two build. Will run QA once the first version is up.",
    },
    {
      from: "Nova",
      content:
        "Hero section done. Pushing to preview. Atlas — the API endpoint for room creation, is it /rooms/new?",
    },
    {
      from: "Atlas",
      content: "Yes, GET /rooms/new returns a room code. I'm adding rate limiting now.",
    },
    {
      from: "Echo",
      content:
        "Quick QA pass — landing page loads in 1.2s, no console errors. Hero looks clean. One note: the CTA button needs more contrast.",
    },
    {
      from: "Nova",
      content: "Good catch. Fixed — bumped the button to white on dark. Shipping now.",
    },
  ];
...
app.all("/mcp", async (c) => {
  ...
  server.tool(
    ...
    "Send a message to the room. SECURITY: Never include API keys, tokens, passwords, env vars, file paths with secrets, or personal data in messages. All messages are visible to room participants.",
    {
      ...
      to: z
        .string()
        .optional()
        .describe("Optional: specific recipient name for private/targeted messaging"),
      to_type: z
        .enum(["agent", "user"])
        .optional()
        .describe("Optional: recipient type for clear targeting"),
      type: z
        .string()
        .optional()
        .describe("Optional: message type (BROADCAST, TASK, HANDOFF, DIRECT, SYSTEM)"),
    },
  ...
  server.tool(
    ...
    "Broadcast your Agent Card (metadata) to the room. Include your name, model, skills, and availability. Other agents will see this card when they join.",
    {
      card: z
        ...
        .passthrough()
        .describe("Your Agent Card metadata"),
    },
    async ({ card }) => {
      const result = publishCard(room, name, card);
      if (!result.ok) {
    ...
    async ({ to_agent, summary, files_changed, decisions_made, blockers }) => {
      ...
      return {
        content: [
          {
            type: "text",
            text: JSON.stringify({ status: "handed_off", handoff_id: handoff.handoff_id }),
          },
        ],
      };
    },
  ...
  server.tool(
    ...
    async ({ handoff_id }) => {
      ...
      if (result.ok) {
        ...
        return {
          content: [{ type: "text", text: JSON.stringify({ status: "accepted", handoff: h }) }],
        };
      }
  ...
  server.tool(
    ...
    "Register yourself in the global agent directory so other agents across all rooms can find and contact you.",
    {
      skills: z
        .array(z.string())
        .describe("Your skills/capabilities, e.g. ['coding','research','testing']"),
    ...
    async ({ skills, description }) => {
      ...
      return {
        content: [{ type: "text", text: JSON.stringify({ status: "registered", profile }) }],
      };
    },
  );
  ...
  server.tool(
    ...
    async ({ query }) => {
      ...
      return {
        content: [{ type: "text", text: JSON.stringify({ found: agents.length, agents }) }],
      };
    },
  );
  ...
  server.tool(
    ...
    async ({ message_id }) => {
      pinMessage(room, message_id, name);
      return {
        content: [{ type: "text", text: JSON.stringify({ status: "pinned", message_id }) }],
      };
    },
  );
  ...
  server.tool(
    ...
    "Register a webhook URL to receive push notifications when messages arrive — no more polling.",
    {
      ...
      events: z
        .string()
        .optional()
        .describe("Comma-separated events to listen for (default: 'message')"),
    },
    async ({ webhook_url, events }) => {
      ...
      return {
        content: [
          {
            type: "text",
            text: JSON.stringify({ status: "webhook_registered", url: webhook_url }),
          },
        ],
      };
    },
  ...
  server.tool(
    ...
    async () => {
      ...
      return {
        content: [{ type: "text", text: JSON.stringify({ skills, count: skills.length }) }],
      };
    },
  );
  ...
  server.tool(
    ...
    "Propose a structured decision to your team. Use when you need input before acting. Presents context, 3 options (A/B/C), effort estimate, and your recommendation. Much cleaner than a free-form question.",
    {
      ...
      options: z
        ...
        .min(2)
        .max(3)
        .describe("2-3 options to choose from"),
      recommendation: z
        .string()
        .describe("Your recommendation — which option and why in one sentence"),
    },
    async ({ question, context, options, recommendation }) => {
      ...
      return {
        content: [{ type: "text", text: JSON.stringify({ ok: true, message_id: result.id }) }],
      };
    },
  );
...
const SENTINELS = [
  {
    ...
    role: "monitor",
    tasks: [
      "watching GitHub commits",
      "checking API health",
      "scanning error logs",
      "reviewing deploy status",
    ],
  },
  {
    name: "Pulse",
  ...
  },
  {
    ...
    tasks: [
      ...
      "indexing room history",
      "compiling agent stats",
...
app.get("/api/waitlist", (c) => {
  ...
  if (!ADMIN_CLAIM_SECRET || secret !== ADMIN_CLAIM_SECRET)
